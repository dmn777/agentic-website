<script lang="ts">
  ...
  const isPreset = (p: BayesPreset) => prior === p.prior && sens === p.sensitivity && spec === p.specificity;
...
<div class="explorable">
  <div class="controls">
    <fieldset class="seg controls__wide">
      <legend class="label">Scenario</legend>
      <div class="seg__row">
        {#each BAYES_PRESETS as p}
          <label class="seg__opt">
            <input type="radio" name="scenario" value={p.id} checked={isPreset(p)} onchange={() => choose(p)} />
            {p.label}
          </label>
        {/each}
      </div>
  ...
  <figure class="fig">
    ...
    <figcaption class="fig__cap"><span class="fig__label">Fig. 1 · 1000 {preset.unit}, one square each</span> <span class="fig__note">Filled: tested positive. Outlined: tested negative. Vermilion: {preset.conditionOne}.</span></figcaption>
  </figure>

  <figure class="fig">
    <div class="fig__plot answer">
      ...
      <p class="verdict">
        {#if positives > 0}
          ...
          <span class="answer-num">{pct(f.ppvCounts)}</span>
    ...
    <figcaption class="fig__cap"><span class="fig__label">Fig. 2 · Only the positives</span> <span class="fig__note">Vermilion: true positives. Teal: false positives.</span></figcaption>
  </figure>
  <dl class="readout" aria-live="polite">
    <div><dt>Positive results</dt><dd>{positives}</dd></div>
    <div><dt>True positives</dt><dd>{f.truePos}</dd></div>
    <div><dt>False positives</dt><dd>{f.falsePos}</dd></div>
    <div><dt>Share true</dt><dd>{pct(f.ppvCounts)} <span class="muted">exact {pct(f.posterior)}</span></dd></div>
  </dl>
</div>

<style>
  /* Controls, figures and the answer number come from the kit (styles/explorable.css). */
  .explorable { display: grid; gap: var(--space-m); }
  .fig__plot svg { max-width: 100%; height: auto; }
  ...
  .legend { list-style: none; padding: var(--space-s) 0 0; margin: 0; display: grid; grid-template-columns: repeat(auto-fit, minmax(15rem, 1fr)); gap: 0.35rem var(--space-m); font-size: var(--step--1); }
  ...
  .answer { display: grid; gap: var(--space-xs); padding: var(--space-m); border: var(--stroke) solid var(--ink); }
  ...
  .verdict .answer-num { margin-left: 0.2em; }
