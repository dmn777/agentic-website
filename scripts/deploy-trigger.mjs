...
// repository_dispatch "sanity-publish" event (Contents permission), the same event a
// Sanity webhook would send. With --verify it then runs qa:deploy for the new run, passing
// on the remaining flags. The token is read from the private secrets file, never printed.
